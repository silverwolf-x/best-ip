import { normalizeIpureScores, IPURE_SCORE_LABELS, ipureScoreInlineStyle, formatAsn, escapeHtml, normalizeUnavailableLatencyStatus } from "../results.js";
...
function renderDetails(result) {
  ...
  const scoreVal = result.score == null || result.score === "" ? null : Number(result.score);
  const scoreStyle = ipureScoreInlineStyle(scoreVal);
  const scoreBadgeCls = scoreStyle ? "score-band" : "score-none";
  const coffeeScore = Number.isFinite(result.coffee_score) ? result.coffee_score : null;
  // Coffee 评分不是 IPure 的分数，沿用应用原有的三档配色，不套用 ipure.dev 的色带。
  ...
  headSection.innerHTML = `
    <div class="modal-hero">
      <div class="modal-hero-main">
        ...
        ${ipureReportUrl ? `<div class="modal-tag-row"><a class="chip chip-asn" href="${escapeHtml(ipureReportUrl)}" target="_blank" rel="noreferrer noopener">IPure 原始报告</a></div>` : ""}
      </div>
