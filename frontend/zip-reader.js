(() => {
  ...
  async function readArtifact(buffer, identity) {
    ...
    if (
      result?.id !== identity.requestId ||
      result?.status !== "completed" ||
      result?.manifest_ready !== true ||
      !Array.isArray(result?.results) ||
      !result?.manifest ||
      result.manifest.job_id !== identity.requestId ||
    ...
    result.results.forEach((record, index) => {
      if (
        !record ||
        record.job_id !== identity.requestId ||
  ...
  window.BestIpZip = Object.freeze({ readArtifact });
})();
