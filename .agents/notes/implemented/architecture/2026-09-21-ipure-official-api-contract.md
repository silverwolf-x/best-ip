...
用 `.env` 的真实订阅连续扫描，失败节点的集合**每次运行都不同**，且节点服务器会主动重置连接
（Mihomo 日志：`wsarecv: An existing connection was forcibly closed by the remote host`）。
失败节点的 `page` 与 `trace` 同时停在 ~5003ms，即连接超时而非业务错误。

实测并发数与失败数没有稳定关系（并发 8 → 失败 7；并发 4 → 失败 4；并发 2 → 失败 3；并发 1 → 失败 6），
提高单节点尝试次数（6 次）也只能把覆盖从 18/25 提升到 21/25。隔离单跑同一节点同样会重置。
结论：这是订阅方节点质量/限流问题，本地参数调不出"零失败"。
...
本轮 25 节点订阅的最佳一次为 22/25（并发 2），覆盖 22/22；其余失败全部是节点侧连接重置。
