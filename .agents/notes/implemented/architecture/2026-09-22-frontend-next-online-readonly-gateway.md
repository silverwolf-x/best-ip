...
同时，这一版页面被用户定的硬约束框住：**输入控件只允许**搜索框、状态筛选、排序切换、两个导出按钮、行内复制；表头 `thead` 内不得有任何控件；选任务只走 URL 参数。也就是说「在页面里放一个开始扫描的按钮」这条路是关着的——扫描只能由页面之外发起（Actions 的 `workflow_dispatch` / 脚本），页面必须是一个**只读**的查看器。
...
线上页面（`wrangler.jsonc` 的 `assets.directory` 已指向 `./frontend-next`）打开后走一条只读通路：问本站 Worker「最近一次有结果的扫描是哪一次」，拿它给的签名地址直连取字节，本地校验后渲染。扫描的发起仍在页面之外。
...
- `/site-config.js` 不再在缺 `SCAN_KEY_ID` 时 503：这个文件是「这次部署怎么跑」的标记，不是「扫描能力齐不齐」的检查。它 503 的后果是页面读不到配置 → 退回示例数据 → 线上安静地显示一屏合成 IP，一个配置疏漏装成了设计示例。能力检查留给 `/api/*` 的 `assertWorkerConfigured`（那里缺配置就该明确 503）。`publicKeyPath` / `keyId` 字段保留（`keyId` 不合法时为 `null`），因为只有那个会发起扫描的旧前端用得到，回退路径还要靠它。

## Alternatives considered

### Why not 在页面里放「开始扫描」按钮（A2 路线）？

最强理由：功能上最完整——用户不用离开页面就能发起一次扫描，触发与查看在同一处，也省掉「谁去点 Actions」这一步。不选的原因：这一版页面的输入控件白名单是用户亲口定死的硬约束（搜索、状态、排序、两个导出、行内复制），而「开始扫描」不在其中，还得配上订阅输入框、进度区与 `scan_token` 的存取——那等于把旧前端被判定为垃圾的东西请回来。**违反硬约束比少一个入口更糟**。触发继续留在页面之外（Actions `workflow_dispatch` / 脚本）。

### Why not 复用旧前端的 `frontend/src/transport/gateway.js`？

最强理由：那条通路已经在线上跑过，订阅信封加密、`POST /api/scans`、轮询、取产物全都写好了，共享一份就没有两套实现漂移的问题。不选的原因有两条：一是它整条链都围着「发起这次扫描的人」转（订阅 URL、`keyId`、`scan_token`、进度面板），而新页面不发起扫描，复用它就得把 crypto 与信封那套也拖进来，与「零第三方依赖、打开即渲染」冲突；二是生产只能有一个 `assets.directory`，跨目录的相对 import 在线上是 404，所以「共享」在无打包器的静态站里根本落不了地。
...
最强理由：用户常常是「发一次扫描 → 打开页面」，这时页面显示的是上一次结果，能提示「新一次正在跑」会很有用。不选的原因：这需要在 payload 里加一个「最新运行的状态」字段，再在页面上找一处显示它——而页面上的输入控件与状态区同样受硬约束约束。当前取舍是：badge 明确写「最近一次**有结果**的扫描」，`running` / `queued` 只在最新 5 次 completed 都没有产物时才成为答案。这是**已知的信息缺口**，不是遗漏；要做就得先放开「允许加一处状态文字」。
...
- **收益**：线上页面一打开就是最近一次**有结果**的扫描，九项内容一屏可见；页面零新增控件，仍是纯只读查看器。
...
- **代价**：旧 `frontend/` 不再被提供，**页面内的「发起扫描」入口随之消失**，扫描只能由 Actions `workflow_dispatch` / 脚本发起。这是「页面控件白名单」与「进入生产」两条要求叠出来的必然结果，仓库文件未删，回退一行即恢复。
