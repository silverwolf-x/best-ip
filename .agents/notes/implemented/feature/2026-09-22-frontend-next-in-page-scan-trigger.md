...
- **代价**：页面控件从 5 个变成 8 个，只读查看器变成带操作的表单；`subscriptionUrl` 是明文输入（用户要求不做隐私掩码，2026-09-23 从 `type="password"` 改成 `type="text"`，图标同步从挂锁换成链接），它在 DOM 里始终是明文值——**这是操作入口的固有代价，不是疏漏**。页面不回显、不落盘、不进导出快照。
