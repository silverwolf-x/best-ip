...
- **已知缺口**：线上解密能力无法在本机直接证伪——私钥在 GitHub secret `SCAN_PRIVATE_KEY_PEM` 里。现有补证是一对合成 RSA-3072 密钥的真加密真解密（见 Testing），**线上那一次的端到端解密仍然只有生产里真跑一次才能证**。
