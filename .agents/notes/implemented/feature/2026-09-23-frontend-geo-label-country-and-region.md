...
- **`5990163` 上线后自证**：`Deploy Cloudflare Worker` run `35855136253` 第一次判红（`结果：6/7 项通过`），红因是**边缘传播窗口**——线上还是旧字节（`app/render.js` 18393 B、`index.html` 7111 B，而 HEAD 是 18757 / 7120），同一 run 重跑即 `7/7`、`线上内容与 HEAD 逐字节一致（18/18 个文件）`，所以线上确实在提供这一版文案。这条门禁为什么只在探针 404 时等待、以及这次为什么会撞上窗口，见 [发布后自证笔记](../process/2026-09-21-post-deploy-self-verification.md)。
