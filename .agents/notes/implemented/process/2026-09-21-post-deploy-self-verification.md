...
- **这条等待只认 404，因此有一个已实测的边界**：探针探的是「文件在不在」。只改**已存在**文件的发布里，探针文件在旧版本里也有，第一次探就是 200，等待立刻结束、比对撞进窗口——2026-09-23 的 `5990163`（只改 `app/render.js`、`index.html` 的文案）就是这样：线上 `app/render.js` 18393 B（旧）≠ HEAD 18757 B、`index.html` 7111 B（旧）≠ 7120 B，`结果：6/7 项通过`（run `35855136253`）。同一 commit 重跑即 `7/7`、`18/18 个文件` 一致，所以那次红是传播窗口，不是漂移。要修的方向是让「不一致」也进等待循环（每轮要求**全部**文件一致，收敛即通过、等满仍不一致才判红）——那会改变这条门禁的判据语义，得单开一篇决定，本轮只把边界记在这里。
...
- 发布实录（只改**已存在**文件的发布，2026-09-23）：`5990163` 的 `workflow_run` run `35855136253` 第一次判红（`结果：6/7 项通过`，红因正是上一条那个边界），同一 run 重跑 success（`7/7`、`线上内容与 HEAD 逐字节一致（18/18 个文件）`）；`4f96c5f`（只改笔记）的那次 `workflow_run` 被我这次重跑按并发组取消，于是用 `workflow_dispatch` 补跑 run `35855455402` success。这次发布带上去的是新前端的表头文案，见 [归属地列表头写「国家和地区」](../feature/2026-09-23-frontend-geo-label-country-and-region.md)。
