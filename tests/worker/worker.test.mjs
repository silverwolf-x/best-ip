...
import { assertSameOrigin, authenticate, signScanToken, verifyScanToken } from "../../worker/auth.js";
import { assertWorkerConfigured } from "../../worker/config.js";
import { exactRun, listRunJobs, validateEnvelope } from "../../worker/scans.js";
import { findArtifact, limitStream } from "../../worker/artifacts.js";
...
test("validateEnvelope accepts the browser envelope and rejects boundary violations", () => {
  ...
  const valid = validateEnvelope({ request_id: requestId, key_id: keyId, envelope: envelope(requestId) }, baseEnv);
  assert.equal(valid.requestId, requestId);
  assert.equal(valid.keyId, keyId);
  assert.throws(() => validateEnvelope({ request_id: requestId, key_id: "b".repeat(64), envelope: envelope(requestId) }, baseEnv), /指纹/);
  assert.throws(() => validateEnvelope({ request_id: requestId, key_id: keyId, envelope: envelope(requestId, { expires_at: Math.floor(Date.now() / 1000) }) }, baseEnv), /过期/);
  assert.throws(() => validateEnvelope({ request_id: requestId, key_id: keyId, envelope: envelope(requestId, { ct: "" }) }, baseEnv), /字段/);
});
...
test("state-changing requests require same-origin metadata", () => {
  ...
  assert.throws(() => assertSameOrigin(crossOrigin), (error) => error.status === 403 && error.code === "csrf_rejected");
});

test("health configuration is fail-closed", () => {
  assert.throws(() => assertWorkerConfigured({}), (error) => error.status === 503 && error.code === "worker_not_configured");
  assert.doesNotThrow(() => assertWorkerConfigured({ SCAN_KEY_ID: keyId, SCAN_TOKEN_SECRET: "secret", GITHUB_APP_ID: "1", GITHUB_APP_INSTALLATION_ID: "2", GITHUB_APP_PRIVATE_KEY: "pem" }));
});

test("scan token is bound to request identity and cannot be forged or expired", async () => {
  const requestId = "req-token-1";
  const token = await signScanToken(baseEnv, requestId, Date.now());
  const payload = await verifyScanToken(baseEnv, token, requestId);
  assert.equal(payload.request_id, requestId);
  await assert.rejects(verifyScanToken(baseEnv, `${token}x`, requestId), /无效/);
  await assert.rejects(verifyScanToken(baseEnv, token, "req-other"), /无效/);
  ...
  await assert.rejects(verifyScanToken(baseEnv, `${expiredPayload}.${signature}`, requestId), /过期|无效/);
...
test("Access authentication fails closed and enforces the configured email", async () => {
  ...
  await assert.rejects(authenticate(request, {}), (error) => error.status === 503);
  ...
  const nativeAccepted = await authenticate(request, nativeEnv, nativeContext);
  ...
  await assert.rejects(authenticate(request, env), (error) => error.status === 401);
  ...
  try {
    ...
    await assert.rejects(authenticate(denied, env), (error) => error.status === 403);
    ...
    const configuredNativeAccepted = await authenticate(request, env, configuredNativeContext);
    ...
    await assert.rejects(
      authenticate(new Request(request, { headers: { "cf-access-jwt-assertion": wrongAlgorithm } }), env),
      (error) => error.status === 401,
    );
  } finally {
...
test("exactRun binds request title, dispatch event, branch and time window", () => {
  ...
  assert.equal(exactRun(run, requestId, dispatchedAt), true);
  assert.equal(exactRun({ ...run, path: ".github/workflows/unrelated.yml" }, requestId, dispatchedAt), false);
  assert.equal(exactRun({ ...run, display_title: "other" }, requestId, dispatchedAt), false);
  assert.equal(exactRun({ ...run, path: undefined }, requestId, dispatchedAt), false);
});
...
test("GitHub App installation token is used for paginated, attempt-filtered jobs", async () => {
  ...
  try {
    const result = await listRunJobs(env, { id: 55, run_attempt: 2 });
...
test("artifact matching rejects ambiguous names and accepts one exact artifact", async () => {
  ...
  try {
    const artifact = await findArtifact(env, { id: 8, run_attempt: 1 }, "req-art");
    assert.equal(artifact.id, 1);
    duplicate = true;
    await assert.rejects(findArtifact(env, { id: 8, run_attempt: 1 }, "req-art"), /唯一/);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("artifact streaming enforces the byte cap without buffering the whole archive", async () => {
  const allowed = limitStream(new Response(new Uint8Array([1, 2, 3])).body, 3);
  const allowedBytes = new Uint8Array(await new Response(allowed).arrayBuffer());
  assert.deepEqual([...allowedBytes], [1, 2, 3]);

  const oversized = limitStream(new Response(new Uint8Array([1, 2, 3])).body, 2);
