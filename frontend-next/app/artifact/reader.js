/* ============================================================================
   产物读取器 —— frontend/src/artifact/reader.js 的逐字副本（外加 validation.js）
   ----------------------------------------------------------------------------
   ...
   两边必须同时改：改动 frontend/src/artifact/ 里任何一个文件时，把同名文件一起同步过来。
   reader.js 是入口（它 import ./validation.js），校验规则与 IPure 取值域都在那一边。
